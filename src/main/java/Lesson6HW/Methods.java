...
public class Methods {
    public static void manufacturerFind(ArrayList<Notebook> book) {
        String msg = "Введите марку: \n";
        String inputValue = enterFromUser(msg);
        int count = 0;
        for (Notebook item : book) {
            String manufacturer = item.getManufacturer();
            if (manufacturer.equals(inputValue)) {
                item.printNoteBooks();
                count++;
            }
        }
        if (count == 0) {
            System.out.printf("Марка \"%s\" не найдена.%n", inputValue);
        }
    }

    public static void ramFind(ArrayList<Notebook> book) {
        String msg = "Введите минимальный объём оперативной памяти в MB: \n";
        double inputValue = checkDouble(msg);
        int count = 0;
        for (Notebook item : book) {
            double ram = item.getRam();
            if (ram >= inputValue) {
                item.printNoteBooks();
                count++;
            }
    ...
    public static void hardDiskFind(ArrayList<Notebook> book) {
        String msg = "Введите минимальный объём жёсткого диска в GB: \n";
        double inputValue = checkDouble(msg);
        int count = 0;
        for (Notebook item : book) {
    ...
    public static void diagonalFind(ArrayList<Notebook> book) {
        ...
        int count = 0;
        for (Notebook item : book) {
            double diagonal = item.getDiagonal();
    ...
    public static void allParametersFind(ArrayList<Notebook> book) {
        String msg1 = "Введите марку: \n";
        String inputValue1 = enterFromUser(msg1);

        String msg2 = "Введите минимальный объём оперативной памяти в MB: \n";
        double inputValue2 = checkDouble(msg2);

        String msg3 = "Введите минимальный объём жёсткого диска в GB: \n";
        ...
        for (Notebook item : book) {
            ...
            double hardDisk = item.getHardDisk();
            String color = item.getColor();
            double diagonal = item.getRam();

            if (manufacturer.equals(inputValue1)
                    && ram >= inputValue2
